...
app.post("/api/payments/create",async(req,res)=>{
  try{
    ...
    // Standard UPI intent URI. Keep the payload simple for maximum scanner compatibility.
    const vpa=String(s.upi_id).trim().replace(/\\s+/g,"");
    const payee=String(s.organization_name||"Fee Payment").trim().slice(0,80);
    const amountText=Number(amount).toFixed(2);
    const upi=`upi://pay?pa=${encodeURIComponent(vpa)}&pn=${encodeURIComponent(payee)}&am=${encodeURIComponent(amountText)}&cu=INR&tr=${encodeURIComponent(ref)}&tn=${encodeURIComponent(ref)}`;
    const qr_url=await QRCode.toDataURL(upi,{
    ...
    res.json({payment_reference:ref,amount,upi_url:upi,upi_id:vpa,qr_url,qr_payload:upi});
